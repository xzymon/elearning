...
public class DocServiceBean implements DocServiceBusiness {
	...
	public Doc ownerDownloadDoc(String ownerNick, String fileName, Date date) {
		if(ownerNick!=null && fileName != null & date != null){
			TypedQuery<Doc> tquery = em.createQuery("from Doc doc join fetch doc.binaryData binaryData where doc.owner.nickName=:ownerNick and doc.fileName=:fileName and doc.uploadTime=:date", Doc.class);
